...
ELSE goods.price
 END AS actual_price
FROM learning.goods 
LEFT JOIN learning.season_price  ON  goods.good_id = season_price.good_id;
...
CASE
WHEN sp.validity_from <= NOW() AND (sp.validity_to IS NULL OR sp.validity_to >= NOW()) THEN g.price - (g.price * g.discount / 100)
ELSE g.price
END
 ELSE 0
END AS actual_price,
(SELECT MAX(discount) FROM learning.goods
 WHERE (validity_from <= NOW() AND (validity_to IS NULL OR validity_to >= NOW()))) AS bigger_discount
FROM learning.goods g
LEFT JOIN learning.season_price sp ON g.good_id = sp.good_id;
